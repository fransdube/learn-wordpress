...
final class UriInfo
{
    ...
    {
        return $uri instanceof \GFPDF_Vendor\Psr\Http\Message\UriInterface ? '' : null;
    }
    /**
    ...
    {
        if ($uri instanceof \GFPDF_Vendor\Psr\Http\Message\UriInterface || $uri instanceof \GFPDF_Vendor\League\Uri\Contracts\UriInterface) {
            return $uri;
        }
    ...
    private static function normalize($uri)
    {
        ...
        if ('/' === ($path[0] ?? '') || '' !== $uri->getScheme() . $uri->getAuthority()) {
            $path = \GFPDF_Vendor\League\Uri\UriResolver::resolve($uri, $uri->withPath('')->withQuery($null))->getPath();
        }
        $query = $uri->getQuery();
        ...
        return $uri->withHost(\GFPDF_Vendor\League\Uri\Uri::createFromComponents(['host' => $uri->getHost()])->getHost())->withPath($path)->withQuery([] === $pairs ? $null : $query)->withFragment($null === $fragmentOrig ? $fragmentOrig : $fragment);
    }
    /**
    ...
    public static function isSameDocument($uri, $base_uri) : bool
    {
        ...
        return (string) $uri->withFragment($uri instanceof \GFPDF_Vendor\Psr\Http\Message\UriInterface ? '' : null) === (string) $base_uri->withFragment($base_uri instanceof \GFPDF_Vendor\Psr\Http\Message\UriInterface ? '' : null);
    }
    /**
    ...
    public static function getOrigin($uri) : ?string
    {
        ...
        if ('blob' === $scheme) {
            $uri = \GFPDF_Vendor\League\Uri\Uri::createFromString($uri->getPath());
