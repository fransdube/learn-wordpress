...
final class Authority extends \GFPDF_Vendor\League\Uri\Components\Component implements \GFPDF_Vendor\League\Uri\Contracts\AuthorityInterface
{
    ...
    public function __construct($authority = null)
    {
        ...
        $this->host = new \GFPDF_Vendor\League\Uri\Components\Host($components['host']);
        $this->port = new \GFPDF_Vendor\League\Uri\Components\Port($components['port']);
        $this->userInfo = new \GFPDF_Vendor\League\Uri\Components\UserInfo($components['user'], $components['pass']);
        $this->validate();
    }
    /**
    ...
    private function validate() : void
    {
        if (null === $this->host->getContent() && null !== $this->getContent()) {
            throw new \GFPDF_Vendor\League\Uri\Exceptions\SyntaxError('A non-empty authority must contains a non null host.');
        }
    }
    /**
     * {@inheritDoc}
     ...
     */
    public function withHost($host) : \GFPDF_Vendor\League\Uri\Contracts\AuthorityInterface
    {
     ...
     */
    public function withPort($port) : \GFPDF_Vendor\League\Uri\Contracts\AuthorityInterface
    {
        if (!$port instanceof \GFPDF_Vendor\League\Uri\Components\Port) {
            $port = new \GFPDF_Vendor\League\Uri\Components\Port($port);
        }
     ...
     */
    public function withUserInfo($user, $pass = null) : \GFPDF_Vendor\League\Uri\Contracts\AuthorityInterface
    {
        $userInfo = new \GFPDF_Vendor\League\Uri\Components\UserInfo($user, $pass);
