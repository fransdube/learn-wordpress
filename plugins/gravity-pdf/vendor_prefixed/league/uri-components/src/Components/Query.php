...
use Iterator;
use GFPDF_Vendor\League\Uri\Contracts\QueryInterface;
...
use GFPDF_Vendor\League\Uri\QueryString;
...
final class Query extends \GFPDF_Vendor\League\Uri\Components\Component implements \GFPDF_Vendor\League\Uri\Contracts\QueryInterface
{
    /**
    ...
    {
        $this->pairs = \GFPDF_Vendor\League\Uri\QueryString::parse($query, $separator, $enc_type);
    ...
    public static function createFromParams($params = [], string $separator = '&') : self
    {
        if ($params instanceof self) {
            return new self(\http_build_query($params->params(), '', $separator, \PHP_QUERY_RFC3986), $separator, \PHP_QUERY_RFC3986);
        }
        if ($params instanceof \Traversable) {
            $params = \iterator_to_array($params, \true);
        }
        if ([] === $params) {
        ...
        return new self(\http_build_query($params, '', $separator, \PHP_QUERY_RFC3986), $separator, \PHP_QUERY_RFC3986);
    }
    /**
    ...
    {
        $query = \GFPDF_Vendor\League\Uri\QueryString::build($pairs, $separator, \PHP_QUERY_RFC3986);
    ...
    public static function createFromUri($uri) : self
    {
        ...
        if (!$uri instanceof \GFPDF_Vendor\Psr\Http\Message\UriInterface) {
            throw new \TypeError(\sprintf('The object must implement the `%s` or the `%s` interface.', \GFPDF_Vendor\Psr\Http\Message\UriInterface::class, \GFPDF_Vendor\League\Uri\Contracts\UriInterface::class));
        }
    ...
    {
        return \GFPDF_Vendor\League\Uri\QueryString::build($this->pairs, $this->separator, \PHP_QUERY_RFC3986);
    }
    /**
     * {@inheritDoc}
     */
    ...
    {
        return \GFPDF_Vendor\League\Uri\QueryString::build($this->pairs, $this->separator, \PHP_QUERY_RFC1738);
    }
    /**
     * {@inheritDoc}
     */
    ...
    {
        $this->params = $this->params ?? \GFPDF_Vendor\League\Uri\QueryString::convert($this->pairs);
        if (null === $key) {
            return $this->params;
        }
     ...
     * {@inheritDoc}
     */
    public function withSeparator(string $separator) : \GFPDF_Vendor\League\Uri\Contracts\QueryInterface
    {
     ...
     * {@inheritDoc}
     */
    public function sort() : \GFPDF_Vendor\League\Uri\Contracts\QueryInterface
    {
     ...
     * {@inheritDoc}
     */
    public function withoutDuplicates() : \GFPDF_Vendor\League\Uri\Contracts\QueryInterface
    {
        if (\count($this->pairs) === \count(\array_count_values(\array_column($this->pairs, 0)))) {
            return $this;
        }
     ...
     * {@inheritDoc}
     */
    public function withoutEmptyPairs() : \GFPDF_Vendor\League\Uri\Contracts\QueryInterface
    {
     ...
     * {@inheritDoc}
     */
    public function withoutNumericIndices() : \GFPDF_Vendor\League\Uri\Contracts\QueryInterface
    {
     ...
     */
    public function withPair(string $key, $value) : \GFPDF_Vendor\League\Uri\Contracts\QueryInterface
    {
     ...
     */
    public function merge($query) : \GFPDF_Vendor\League\Uri\Contracts\QueryInterface
    {
        $pairs = $this->pairs;
        foreach (\GFPDF_Vendor\League\Uri\QueryString::parse(self::filterComponent($query), $this->separator, \PHP_QUERY_RFC3986) as $pair) {
    ...
    {
        if ($value instanceof \GFPDF_Vendor\League\Uri\Contracts\UriComponentInterface) {
     ...
     * {@inheritDoc}
     */
    public function withoutPair(string ...$keys) : \GFPDF_Vendor\League\Uri\Contracts\QueryInterface
    {
        if ([] === $keys) {
            return $this;
        }
        ...
        $filter = static function (array $pair) use($keys_to_remove) : bool {
            return !\in_array($pair[0], $keys_to_remove, \true);
        };
        $new = new self();
     ...
     * @param mixed|null $value
     */
    public function appendTo(string $key, $value) : \GFPDF_Vendor\League\Uri\Contracts\QueryInterface
    {
     ...
     */
    public function append($query) : \GFPDF_Vendor\League\Uri\Contracts\QueryInterface
    {
        if ($query instanceof \GFPDF_Vendor\League\Uri\Contracts\UriComponentInterface) {
            $query = $query->getContent();
        }
        $pairs = \array_merge($this->pairs, \GFPDF_Vendor\League\Uri\QueryString::parse($query, $this->separator, \PHP_QUERY_RFC3986));
        if ($pairs === $this->pairs) {
            return $this;
        }
        $new = new self();
     ...
     * {@inheritDoc}
     */
    public function withoutParam(string ...$keys) : \GFPDF_Vendor\League\Uri\Contracts\QueryInterface
    {
        if ([] === $keys) {
            return $this;
        }
