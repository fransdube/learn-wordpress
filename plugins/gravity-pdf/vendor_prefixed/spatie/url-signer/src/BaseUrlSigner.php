...
use DateTime;
use GFPDF_Vendor\League\Uri\Http;
...
abstract class BaseUrlSigner implements \GFPDF_Vendor\Spatie\UrlSigner\UrlSigner
{
    /**
    ...
    {
        if ($signatureKey == '') {
            throw new \GFPDF_Vendor\Spatie\UrlSigner\Exceptions\InvalidSignatureKey('The signature key is empty');
        }
    ...
    {
        $url = \GFPDF_Vendor\League\Uri\Http::createFromString($url);
     ...
     */
    protected function signUrl(\GFPDF_Vendor\Psr\Http\Message\UriInterface $url, $expiration, $signature)
    {
        $query = \GFPDF_Vendor\League\Uri\QueryString::extract($url->getQuery());
     ...
     * @return UriInterface
     */
    protected function getIntendedUrl(\GFPDF_Vendor\Psr\Http\Message\UriInterface $url)
    {
    ...
    protected function getExpirationTimestamp($expiration)
    {
        ...
        if (!$expiration instanceof \DateTime) {
            throw new \GFPDF_Vendor\Spatie\UrlSigner\Exceptions\InvalidExpiration('Expiration date must be an instance of DateTime or an integer');
        }
        if (!$this->isFuture($expiration->getTimestamp())) {
            throw new \GFPDF_Vendor\Spatie\UrlSigner\Exceptions\InvalidExpiration('Expiration date must be in the future');
        }
     ...
     * @return bool
     */
    protected function hasValidSignature(\GFPDF_Vendor\Psr\Http\Message\UriInterface $url)
    {
        $query = \GFPDF_Vendor\League\Uri\QueryString::extract($url->getQuery());
        $expiration = $query[$this->expiresParameter];
    ...
    protected function buildQueryStringFromArray(array $query)
    {
        ...
        return \GFPDF_Vendor\League\Uri\QueryString::build($buildQuery);
    }
}
